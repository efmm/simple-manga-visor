...
class ImagenScreen(BoxLayout):
	...
	def previous(self):
		if(self.indice==0):
			...
			self.clear_widgets()
			img = ImagenScreen()
			img.put_image(self.lista, self.indice -1)
			self.add_widget(img)
...
class ImagenScreen_archivo(BoxLayout):
	...
	def previous(self):
		if(self.indice==0):
			...
			self.clear_widgets()
			img = ImagenScreen_archivo()
			img.put_image(self.lista, self.indice -1, self.archivo)
			self.add_widget(img)
...
class MenuScreen(StackLayout):
	...
	def load_archivo(self, path, filename):
		...
		if(filename==[]):
			...
			elif (rarfile.is_rarfile(filename[0])):
				archivo = rarfile.RarFile(filename[0], mode="r")
				lista = archivo.namelist()
				res = [unicode(f) for f in lista if re.match(u".+(\\.(?i)([jJ][pP][gG]|[pP][nN][gG]|[gG][iI][fF]|[bB][mM][pP]|[jJ][pP][eE][gG]))$", f)]
				if(res==[]):
					error_no_encontrado = Popup(title="Error", content=Label(text="No compatible files inside the file"), size_hint=(0.5,0.5))
